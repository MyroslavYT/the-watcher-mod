...
public class EventManager {
    ...
    // Fake player state machine: 0=offline 1=joined 2=left(about to rejoin) 3=It(permanent, day7+)
    private static int fpState=0;
    private static int fpNextTick=0;
    private static boolean fpAdvGivenThisSession=false;
    ...
    public static void tick(MinecraftServer server){
        ...
        // ── Fake player: join, wait, make an advancement, wait, leave, ~10s later rejoin ──
        tickFakePlayer(server,data,day,player);
    ...
    private static void trySpawnWatcher(ServerWorld world, ServerPlayerEntity player, int day){
        ...
        // Spawn within the player's CURRENT view cone, not somewhere behind them —
        // the point is that they actually see it appear.
        double yawRad=Math.toRadians(player.getYaw());
        double fdx=-Math.sin(yawRad), fdz=Math.cos(yawRad);
        double spread=Math.toRadians((RNG.nextDouble()*50)-25); // within ±25° of dead ahead
        double cosT=Math.cos(spread), sinT=Math.sin(spread);
        double dirX=fdx*cosT-fdz*sinT, dirZ=fdx*sinT+fdz*cosT;
        double sx=player.getX()+dirX*dist;
        double sz=player.getZ()+dirZ*dist;
    ...
    /** Runs the fake player's whole "life cycle": joins, waits, grants an advancement,
     *  waits again, leaves, then rejoins about 10 seconds later — forever, until Day 7
     *  when It joins one last time and never leaves. */
    private static void tickFakePlayer(MinecraftServer server, WatcherWorldData data, int day, ServerPlayerEntity player){
        if(day<3) return;
        if(day>=7){
            if(fpState!=3){
                FakePlayerManager.sendJoin(server,data,7);
                fpState=3;
            }
            return; // permanent — never leaves again
        }
        switch(fpState){
            case 0: // offline
                if(fpNextTick==0) fpNextTick=tick+100+RNG.nextInt(200);
                if(tick<fpNextTick) return;
                FakePlayerManager.sendJoin(server,data,day);
                fpState=1;
                fpAdvGivenThisSession=false;
                fpNextTick = (day>=5&&FakePlayerManager.hasMoreAdvancements(data))
                    ? tick+2400+RNG.nextInt(2400)   // 2-4 min until it "does something"
                    : tick+200+RNG.nextInt(400);    // phase 1: just a short, unsettling blip
                break;
            case 1: // joined
                if(tick<fpNextTick) return;
                if(day>=5&&!fpAdvGivenThisSession&&FakePlayerManager.hasMoreAdvancements(data)){
                    FakePlayerManager.triggerAdvancement(server,data,day);
                    fpAdvGivenThisSession=true;
                    fpNextTick=tick+1200+RNG.nextInt(1200); // leaves 1-2 min after the advancement
                } else {
                    FakePlayerManager.sendLeave(server,data,day);
                    fpState=2;
                    fpNextTick=tick+190+RNG.nextInt(30); // rejoins ~10 seconds later
                }
                break;
            case 2: // left, about to rejoin
                if(tick<fpNextTick) return;
                FakePlayerManager.sendJoin(server,data,day);
                fpState=1;
                fpAdvGivenThisSession=false;
                fpNextTick = (day>=5&&FakePlayerManager.hasMoreAdvancements(data))
                    ? tick+2400+RNG.nextInt(2400)
                    : tick+200+RNG.nextInt(400);
                break;
        }
    }
}
